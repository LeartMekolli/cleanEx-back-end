...
class PostController extends Controller
{
    ...
    public function get_user_posts(){
       ...
       // $user_posts = Post::where('user_id',Auth::user()->id)->;
        //$user = User::where('id',1)->first();
        $posts = Post::where('user_id',Auth::user()->id)->get();
        
        
        if(count($posts) == 0){
            return response()->json(['message'=>'You do not have any posts!'],200);
        }
        
        foreach ($posts as $key => $value) {
            $response[$key] = [
                "id"=> $posts[$key]->id,
                "service_name" => $posts[$key]->service->service_name,
                "created_at" => Carbon::parse($posts[$key]->created_at)->format('d/m/Y H:i'),
                
                "payment_status" => $posts[$key]->payment_status,
